...
- 与模组脚**同名不同写法**的丝印（J4/J5 的丝印 `A10/A11/A12/A13` vs 模组脚名 `IOA10/IOA11/…`）：
  **先量再并**。名字对不上光靠同名归并不行（AGENTS §5 要显式 tie），一开始我就是用 `<buses>` tie 的；
  但**用户 2026-09-16 用万用表实测这两处不通**（中间有串阻/未连接）⇒ 最终**拆成 4 个独立网**。
  ⚠ 教训：`tie` 的前提是"**确实是同一根线**" —— 不通就不要 tie；
  错误的 tie 会让你在图上引一根线却"看不见地"连到了模组脚上（比不连更难发现）。
...
**★ 封装尺寸必须有出处，不许照抄另一个元件（2026-09-17 踩坑，CH340N）**

CH340N（**SOP-8**）的 pcb 当初直接沿用了 `svg/CH340C`（**SOP-16**）的 `row=3.7` 与焊盘 `0.7×2.2`，
只把本体长改成 4.9 —— 结果**焊盘中心跑到引脚末端之外 0.7mm**（用户一眼看出不对）。
“同是 150mil 就照抄”不成立：真正要保证的是「焊盘中心跨距 = 引脚末端跨距」。

取尺寸的顺序：

1. **器件手册的封装表/机械图**。CH340 手册第 1 页只给了「CH340N = SOP-8、塑体宽 3.9mm、脚距 1.27mm」。
2. **厂商官方 PCB 工程**：`D:\Downloads\CH340PCB.ZIP` → `SERIAL/TTL/CH340N4T-R0/…SchDoc`
   里写着 `ModelName=SOP8`、`Description=SOP8`（即 WCH 自己用的就是标准 SOP-8 封装）。
3. **标准 land pattern（IPC-7351）**：SOP-8/150mil 即 `SOIC127P600X175-8N` ——
   脚距 **1.27**、焊盘中心跨距 **6.00**（row=±3.0）、焊盘 **0.60×1.75**、本体 **3.9×4.9**。

自洽校验（写脚本时自检）：丝印本体框必须与焊盘**零相交** ——
SOP-8 框 4.9×3.9（框边 y=±1.95、x=±2.45）vs 焊盘（y 2.125..3.875、x ±(1.905±0.3)）→ 不相交 ✓；
icon 里的引脚伸出长度也要对得上（本例引脚伸 1.05mm → 端点 ±3.0 = 焊盘中心跨距 6.00 的一半 ✓）。

> ✔ **最终口径（2026-09-17 用户给全套嘉立创封装后定稿）：焊盘几何一律以
> 「嘉立创/立创EDA 导出的封装 svg」为准** —— 用户板上的封装就是立创的。
> 立创把焊盘中心放在**引脚中点附近**、焊盘做得更长，所以**焊盘中心跨距比 IPC-7351
> （中心 = 引脚端点）小 0.3~0.7mm**；封装名里的 `LS`（引脚端点跨距）
> **不等于**焊盘中心跨距。两边都能焊，但以用户实际用的那个为准。
>
> 取样即：用户从立创EDA 导出封装 svg → 脚本解析 `c_origin / c_width / c_height`
> （1 单位 = 0.254mm）→ 直接得到「焊盘尺寸 + 焊盘中心跨距」。已按此对齐：
>
> | 元件 | 立创封装名 | 焊盘 | 焊盘中心跨距 | 脚距 |
> |---|---|---|---|---|
> | CH340N | `SOP-8-L4.9-W3.9-P1.27-LS6.0-BL` | 0.630 × 1.865 | **5.364** | 1.27 |
> | CH340C | `SOP-16-L10.0-W3.9-P1.27-LS6.0-BL` | 0.560 × 1.745 | **5.744** | 1.27 |
> | SY8089 | `SOT-23-5-L3.0-W1.7-P0.95-LS2.8-BR` | 1.100 × 0.600 | **2.600** | 0.95 |
> | CN3165 | `DFN-8-L3.0-W3.0-P0.50-BL-EP` | 0.280 × 0.665 | **2.814**（散热盘 2.45×1.65）| 0.50 |
>
> 取证顺序（**第 1 条优先**）：
> 1. **嘉立创/立创EDA 的封装**（用户板子上真正用的）—— 有就直接用，别再自己推；
> 2. 器件手册的封装表/机械图（给的是**芯片**尺寸：本体/脚距/LS，不是 land pattern）；
> 3. IPC-7351 标准盘 —— **只在没有立创封装时**才用（并注明是标准值）。
>
> **元件内交叉校验依然要做**，但要注意：立创口径下 **pcb 焊盘中心跨距 < icon 引脚端点跨距
> 是正常的**（焊盘中心在引脚内侧）；若**反过来大很多**（焊盘中心超出引脚端点）就要怀疑抄错了。
>
> 历史教训：CH340N 曾照抄 CH340C 的 `row=3.7`（两者共用 150mil 族但 SOP-8 ≠ SOP-16），
> 焊盘中心为此跑到引脚末端之外 0.7mm；后来一度按 IPC-7351 改成 3.0 —— 两步都不对，
> **最终按立创定稿 2.6825**。教训：**同族 ≠ 可照抄；有文件出处也不等于跟用户手上的一致**，
> 要拿**用户真正用的那份**封装来对。
...
| 14 | 焊盘跑到引脚外面 / 跨距偏大 | **照抄了另一个元件的 pcb 参数**（CH340N 抄 CH340C 的 row=3.7） | 按标准 land pattern 取：SOP-8(150mil) = `SOIC127P600X175-8N`（脚距 1.27、跨距 6.00、盘 0.60×1.75）；并用丝印框与焊盘「零相交」自检 |
