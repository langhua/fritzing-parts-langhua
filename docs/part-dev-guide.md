...
> ✔ 同类问题已查并修正：`svg/CH340C`（SOP-16，同一 150mil 族）原本也是 `row=3.7` ——
> 那个值**不是抄错**，它来自**立创EDA 导出的封装文件**（`D:\Downloads\SOP-16_2026-09-01.svg`；
> 实测该文件两排焊盘 y 差 29.13 单位 × 0.254mm = **7.399mm**、pitch 1.27mm）。
> 但"**有文件出处 ≠ 数据就对**"：150mil 窄体的引脚端点跨距只有 **6.00mm**，
> 焊盘中心放 3.7 会**超出引脚末端 0.7mm**（焊盘一半悬空），
> 而且与**该元件自己的 icon 自相矛盾**（icon 里引脚只伸到 ±3.0）。
> 2026-09-17 已按标准值改成 `row=3.0` / 焊盘 `0.60×1.75`。
>
> **教训升级**：查出处之外，还要做**元件内交叉校验** —— pcb 的焊盘中心跨距必须等于
> icon 里画出来的引脚端点跨距（同一个生成脚本里两个数要对得上），对不上就是有一边错了。
> （另：`svg/CH340E` 的 `row=2.35` 与它 icon 的引脚端点 ±2.5 自洽，**不用改**。）
