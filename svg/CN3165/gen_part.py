...
def gen_pcb_svg():
    ...
    尺寸来源与推导（手册第 12 页「封装信息」DFN-8 机械图 + 常规外扩）：
      ...
      · 引脚端点 = 本体/2 + L = 1.5 + 0.4 = **1.9**；焊盘中心取「引脚中点外移到 1.70」
        （覆盖 1.30..2.10：内侧盖住引脚 0.2、外侧出引脚端点 0.2），焊盘 **0.28(宽) × 0.80(长)**
      · 散热盘焊盘比芯片盘每侧外扩 0.05 → **2.5 × 1.8**，中心在 (0,0)
      ⚠ 手册**不给 land pattern**，上面这组是按封装尺寸推导的；拿到嘉立创/立创EDA 的
        DFN-8(3×3) 封装后要再校一遍（已记入 docs/part-dev-guide.md 的取证顺序）。
    丝印：本体四角 L 形角标（避开焊盘）+ pin1 实心圆点（本体左下角内）。
    """
    pw, pl = 0.28, 0.80      # 引脚焊盘：宽(x) × 长(y)
    pitch = 0.5              # 脚距（手册）
    row = 1.70               # 上下排焊盘中心 y = ±1.70
    ep_w, ep_h = 2.5, 1.8    # 散热盘焊盘（D1/E1 + 0.05/侧）
    pads = []
