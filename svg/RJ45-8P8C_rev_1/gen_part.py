...
# -*- coding: utf-8 -*-
"""
gen_part.py — 生成 Fritzing 自定义元件 RJ45-8P8C_rev_1（Coorle 8P8C 直插式 PCB 插座，
**短体版**）。

与 RJ45-8P8C 的关系（用户 2026-09-18 定）：**只有本体长度从 27.00 缩到 22.00**，
其余全部相同 —— 本体宽 11.63、插口端斜角 30°、弹片/凹槽/窗口的尺寸与相对位置、
焊盘（2×4 蛇形、外径 1.20 / 孔径 0.70、列距 1.50、行距 2.04）、丝印框 11.68 × 12.20
都不变。因为弹片起点、银色右端、凹槽、窗口都是按「距左右边缘的相对量」定位的，
改 BODY_L 一个常量它们就自动跟着收。
...
PART_ID = "RJ45-8P8C_rev_1"
FZPZ = "RJ45-8P8C_rev_1.fzpz"
...
ICON_LABEL = "RJ45"
TITLE = "RJ45 8P8C PCB Socket (short body, DIP, unshielded)"
LABEL = "J"
PACKAGE = "DIP-8P8C (11.63 x 22.00 mm)"
FAMILY = "RJ45"
...
BODY_W = 11.63                     # 本体宽（图纸）
BODY_L = 22.00                     # 总深：rev_1 = **短体版**（用户 2026-09-18 定；原版 27.00）
...
NOTCH_W = 10.00                    # 凹槽宽固定 10.00（用户 2026-09-18）；rev_1 右缘距右 4.80
...
def gen_breadboard_svg():
    ...
    """
    U = 39.37
    bw, bh = 1100, 900                   # 本体短了（22.00）⇒ 板跟着收（AGENTS §3b：面积尽量最小）
    cx, cy = 550, 550
    y_pin = 100          # 排针靠板端（数字要放**内侧**，见 AGENTS §3b）
    pad_r, hole_r = 1.0 * U, 0.485 * U
    xs = [200 + 100 * i for i in range(N_PINS)]
