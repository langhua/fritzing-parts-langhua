#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""gen_part.py — MX-1.25-3P-V：1.25mm 3P **立贴母座**（板端插座，125 系列）。

数据来源（外部图纸，不入库）：`D:\Downloads\1.25 立贴.PDF`（**125 系列 3P = ZL-125-3AB / 25-125-03A**）。
  · 规格表（DIM A 内跨 / DIM B / DIM C 本体长，随脚数每 +1.25）：
      3P → **A=2.50  B=4.35  C=8.65**（脚距 1.25±0.1）
  · 矢量图实测（用 1.25mm = 34.41pt、端子厚 0.30mm = 8.25pt 双重标定，比例 **27.53 pt/mm**）：
      顶视图（图纸左列第二张，含 6 个脚位）**12.401 × 4.121mm** —— 横向 = 规格表 **DIM C(6P) 12.40** ✓、
      纵向 = 图上 **4.12** ✓；端子 0.30 宽 × 0.82 伸出、左右卡耳外缘 = DIM C 的两端、腔内 3/6 个触点。
      剪成 3P（见下）后：**8.651 × 4.121mm**。
...
# ------------------------------------------------------------------ 身份（包装脚本覆盖）
PART_ID = "MX-1.25-3P-V"
FZPZ = "MX-1.25-3P-V.fzpz"
TITLE = "1.25mm 3P vertical SMD socket (ZL-125-3AB)"
SERIES = "MX1.25"          # 丝印/图标上的系列名（= 市场通用叫法）
MODEL = "ZL-125-3AB"       # 图纸型号（= 25-125-03A）
LABEL = "J"
DATE = "2026-09-24"
...
def fzp_xml():
    ...
    return (
        ...
        ' </properties>\n'
        f' <description>{esc(SERIES)} 1.25mm 3P vertical SMD socket ({esc(MODEL)}), '
