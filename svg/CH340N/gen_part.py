...
# ------------------------------------------------------------------------ pcb
def gen_pcb_svg():
    """PCB 视图（SOP-8 **150mil 窄体**标准盘：IPC-7351 命名 SOIC127P600X175-8N）：

    ★ 两个最容易混的数（务必分清，2026-09-17 用户问到）：
      · **本体（塑体）宽 3.9mm** —— 手册第 1 页封装表给的就是这个数，即 **150mil 窄体**。
        宽体是另一回事：**208mil**（本体 5.28mm，如 svg/W25Q16JV）——**本元件不是那种**。
      · **含引脚的总跨距 6.00mm**（= 本体 3.9 + 两侧引脚各伸出约 1.05）。
        焊盘中心取它的一半 → row = ±3.0。JEDEC MS-012（150mil SOIC）的 E1（引脚端点跨距）
        nominal 就是 6.00mm；对照 208mil 的 W25Q16JV 是 7.9mm（明显更大）。

    其余：脚距 1.27mm、焊盘 0.60(宽,x)×1.75(长,y)、本体 3.9(宽,y)×4.9(长,x)，
    丝印框 4.9×3.9（框边 y=±1.95）与焊盘（内缘 y=2.125）零相交。
    下排 connector0-3（pin1-4）y=+3.0 左→右、上排 connector7-4（pin8-5）y=-3.0 左→右。

    数据来源：
      · 脚距 1.27mm、塑体宽 3.9mm（窄体 150mil）—— CH340 手册第 1 页「3、封装」表
      · WCH 官方 PCB 工程里给 CH340N 用的封装名就是 `SOP8`
        （D:\\\\Downloads\\\\CH340PCB.ZIP → SERIAL/TTL/CH340N4T-R0/…SchDoc 的 ModelName）
      · 跨距 6.00 / 焊盘 1.75：IPC-7351 标准 SOIC-8 land pattern 名义值
