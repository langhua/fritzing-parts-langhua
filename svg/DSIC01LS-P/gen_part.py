...
# ----------------------------------------------------------------------- icon
def gen_icon_svg():
    """贴片 DIP 开关俯视图：黑本体 + 白色拨柄（在上半侧的滑槽里）+ 丝印 ON/1 + 两端焊盘。"""
    ...
    L.append(f'    <rect x="{-SLOT_W / 2:.3f}" y="{-hh + 0.35:.3f}" width="{SLOT_W:.2f}" '
             f'height="{SLOT_H:.2f}" fill="none" stroke="#7a7a7a" stroke-width="0.10"/>\n')
    L.append(f'    <rect x="{-KNOB_W / 2:.3f}" y="{-0.95:.2f}" width="{KNOB_W:.2f}" '
             f'height="{KNOB_H:.2f}" fill="#e8e8e8" stroke="none"/>\n')       # 拨柄（上半侧）
    L.append(f'    <text x="{-hw + 0.35:.2f}" y="{-hh + 1.35:.2f}" font-size="0.85" '
             f'fill="#d0d0d0" font-family="DroidSans">ON</text>\n')
    L.append(f'    <text x="{-hw + 0.35:.2f}" y="{hh - 0.45:.2f}" font-size="0.85" '
             f'fill="#d0d0d0" font-family="DroidSans">1</text>\n')
    L.append('  </g>\n</svg>\n')
    return "".join(L)
...
def gen_breadboard_svg():
    ...
    **上下各 1 个 2.54mm 排针**。
    ...
    L.append(f'  <rect x="{cx - SLOT_W / 2 * U:.1f}" y="{cy - BODY_H / 2 * U + 14:.1f}" '
             f'width="{SLOT_W * U:.1f}" height="{SLOT_H * U:.1f}" fill="none" stroke="#7a7a7a" '
             f'stroke-width="4"/>\n')
    L.append(f'  <rect x="{cx - KNOB_W / 2 * U:.1f}" y="{cy - 37:.1f}" width="{KNOB_W * U:.1f}" '
             f'height="{KNOB_H * U:.1f}" fill="#e8e8e8" stroke="none"/>\n')
