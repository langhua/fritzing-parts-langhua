#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""gen_part.py — SH-1.0-3P-V：1.0mm 3P **立贴母座**（板端插座，SH1.0 系列）。

数据来源（外部图纸，不入库）：`D:\Downloads\1.0 立贴.pdf`（1 页 4 张图）。
  · **抄的是主图纸**：型号 **SH1.0-nP-LT**（图名「SH1.0立贴带盖」）—— 只有它有**俯视图**；
    另两张 `SH1.0-nY` 是**塑壳**承认图、一张是端子图，都没有俯视图（AGENTS §10.15）。
  · 图纸规格表 3P 行：**A=2.00  B=5.00  C=3.60**（脚距 1.00）
  · 矢量图实测（用 1.00mm = 5.625pt、DIM C(7P) 7.60 = 42.6pt 双重标定 ⇒ 比例 **5.625 pt/mm**）：
...
# ------------------------------------------------------------------ 身份（包装脚本覆盖）
PART_ID = "SH-1.0-3P-V"
FZPZ = "SH-1.0-3P-V.fzpz"
TITLE = "1.0mm 3P vertical SMD socket (SH1.0-3P-LT)"
SERIES = "SH1.0"          # 丝印/图标上的系列名（= 市场通用叫法）
MODEL = "SH1.0-3P-LT"     # 图纸主图纸规格表 3P 行（A/B/C = 2.00 / 5.00 / 3.60）
LABEL = "J"
DATE = "2026-09-24"
...
def breadboard_svg():
    ...
    bh = pin_y + 100
    # ★★ 板边相位＝**半格**（AGENTS §3b 新规矩：转接板不许影响板外孔的插拔 ✓，2026-09-27）：
    #   旧做法把板框画成整张画布 ✗ ⇒ 四条边到针脚都是 100（=1 格）的整数倍 ✗
    #   ⇒ 板边正好压在孔行/孔列上 ✗ ⇒ 那一排孔被压掉一半、插不进线 ✗
    #     （实测：`pin33B` 就是被压住的那一个，害得像素板 LED_DIN 多绕 2.5mm ✗）。
    #   现在四周各让 **半格（50 内部单位 = 1.27mm）** ✓ ⇒ 板边落在两排孔**正中** ✓
    #     ⇒ 离最近孔心 1.27mm，谁都不碰 ✓。针脚坐标**一个都不动** ✓（不动就不断老图 ✓）。
    #   副作用：板变小（300×400 = 7.62×10.16mm ✓），正合 §3b“面积尽量最小” ✓。
    INSET = 50
...
def pcb_svg():
    ...
    for cx in ((-MP_PAD_X, MP_PAD_X) if MP_PAD_W > 0 else ()):
        L.append(f'  <rect x="{cx - MP_PAD_W / 2:.3f}" y="{mech_y0:.3f}" '
