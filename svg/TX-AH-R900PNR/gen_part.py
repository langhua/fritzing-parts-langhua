...
# ---- 板上 J5/J4 两个排针（用户 2026-09-19）----
#   共 12 个焊盘：手工版里已挂上 connector57..68pin（几何在手工版，导出表里是 12 个 pad）。
#   本表只定义**名字**与**并到哪条网**。名字按用户 2026-09-19 给的引脚功能：
#     J5（2x4，奇脚在上排、偶脚在下排）：1/2=GND、3=A10、4=CH340E TX、5=A11、6=CH340E RX、7/8=VCC
#     J4（1x4，丝印 GND A13 A12 VCC，编号自右向左）：1=VCC、2=IOA12、3=IOA13、4=GND
#   Axx 并到对应的模组边脚（见 PINS_EDGE）：A10→connector14(IOA10)、A11→15(IOA11)、
#   A12→31(IOA12)、A13→32(IOA13)；TX/RX 是 CH340E 侧的网络，暂不并（待用户确认接到哪条）。
J45_PADS = [
    (57, "GND", None), (58, "A10", 14), (59, "A11", 15), (60, "VCC", None),
    (61, "GND", None), (62, "TX", None), (63, "RX", None), (64, "VCC", None),
...
def icon_svg():
    ...
    for sh in SHAPES:                # ★ 按手工版文档次序画（叠放次序就是画法次序）
        ...
        if kind == "rect":
            ...
            # 金半孔焊盘**保留缺口**：板子四周那 36 个边脚是镀金半孔（castellated，
            # 实物就是「矩形 + 外缘半圆缺口」）。用户 2026-09-18 定：「板子周围的
            # 那些金色焊盘，我还是希望是挖掉半圆的样子，与实物一致」→ 原样输出 d，
            # 不要为了“看上去干净”把它换成实心矩形。
            a = ''
