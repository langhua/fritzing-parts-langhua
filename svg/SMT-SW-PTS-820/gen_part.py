...
# ★★ 2026-09-27（AGENTS §3b 板边相位）：板框改成 x=50..350 ⇒ **画布要跟着加宽 50** ✓
#   （原来画布恰好=板宽 300 ✗ ⇒ 板框右移后会跑出画布 ✗）
BB_VW = BB_W + 50                  # 画布宽（350）
BB_CX, BB_CY = 166, 130            # 开关中心（★ 由 150 右移到 166 ✓）
BB_PIN_Y = 350                     # 排针 y（距下边 100 单位 = 2.54mm）
BB_PIN_XS = [100, 200]             # 两根排针：相距 100 单位 = 2.54mm
#   ★ 为何要把开关右移 ✓：开关端子最远到 cx-104.3 ✓；而"相位合格"的板左边只可能是
#     50 或 -50（针脚在 100/200 ⇒ 左边 ≡ 50 mod 100 ✓）⇒ 取 50 时端子（45.7）会被切掉 4.3 ✗
#     ⇒ 开关中心 150→166 ⇒ 端子 61.7..270.3 ⇒ 稳稳落在 50..350 里 ✓（两侧余 11.7 / 79.7 ✓）。
...
def gen_breadboard_svg():
    ...
    L = ['<?xml version="1.0" encoding="utf-8"?>\n',
         f'<svg xmlns="http://www.w3.org/2000/svg" width="{BB_VW / 100 * 2.54:.2f}mm" '
         f'height="{BB_H / 100 * 2.54:.2f}mm" viewBox="0 0 {BB_VW} {BB_H}">\n',
         ' <g id="breadboard">\n',
         # ★★ 板边相位＝**半格**（AGENTS §3b：转接板不许影响板外孔的插拔 ✓，2026-09-27 ✓）：
         #   排针在 x=100/200 ✓、y=350 ✓ ⇒ **上边本来就是 4.50 ✓ 不能动** ✗；
         #   左/右/下三条是 0.00 ✗ ⇒ 只让这三条 ✓：
         #   x=50、宽 BB_W-100（左右各 50 ✓）、高 BB_H-50（下边 450→400 ✓）。
         f'  <rect x="50" y="0" width="{BB_W}" height="{BB_H - 50}" fill="#00aa44" '
