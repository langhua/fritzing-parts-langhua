...
#   CH32V203C8T6（2026-09-22）：用户手画的面包板里，FPC-05F-12P-H15 的 **12 个触点**就是
#   #c9c9c9（x≈1865.9、19.7×7.9 内部单位，一列 12 个）—— 那是**可见**图元，丢掉图上就少一排
#   触点（`byhand_check.py` 的像素差就是这么来的）。本件的手工版**不含**程序版那些子画法，
#   所以整份跳过规则都可以关掉 ⇒ 给空集合。
SKIP_RECT_FILL_BY_PART = {"CH347T": {"#002d68"}, "CH32V203C8T6": set()}
