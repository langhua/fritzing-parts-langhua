...
| `check_deploy.py [<部件>...] [--mine <dir>] [--fix] [--stray] [--clean-stray]` | **核对「仓库里的元件」与「Fritzing MINE 里装的那份」是否一致** ✓（AGENTS §10 清单第 2 步的机器守，2026-09-27 —— 因"35 件批量让半格"后我逐批报"已部署 ✓"，**独立再量一遍发现 34 件里 9 件根本没到位** ✗ 而写）。★ 它**不假设**命名，而是**从 fzp 自己声明的字段反推** ✓ MINE 该在哪：`moduleId` ⇒ `parts/user/<moduleId>.fzp`；`image="<view>/<名>.svg"` ⇒ `parts/svg/user/<view>/<名>.svg`（这条路径**才是 Fritzing 真正读的那份** ✗）。★ 判据分两层 ✓：**致命 ✗** = fzp 引用到的**图形文件不在那儿 / 是旧的**（写成 `.fzpz` 内部名 `svg.breadboard.*.svg` 就落到这一类 ✗）、fzp 本身旧/缺；**提示（不算 ✗）** = fzp **文件名**与 `moduleId` 不一致 —— Fritzing **自己导入 `.fzpz` 时也会改名** ✓（实测 `SYB-118.fzp` 的 moduleId 是 `Breadboard-SYB118-ModuleID` ✗ 却在正常使用 ✓），一律判错会**误报** ✗。逐条报出**仓库 / MINE 的 mtime**（谁更新一目了然 ✓）。`--fix` 照仓库写到**正确路径** ✓（★ 覆盖前**自动备份**被覆盖的那份到 parts 之外的 `_deploy_backup_<时间戳>/` ✓ —— MINE 是**用户的** Fritzing 目录 ⇒ 必须留后路 ✗）；`--diff` 先把「旧」的**实际差异**打出来 ✓（同步前先看清谁改了什么，别把用户在 Fritzing 里改过的东西冲掉 ✗）；`--clean-stray` 把 `.fzpz` 内部名的残留**移到 parts 之外的备份目录** ✓（**不删** ✗）。退出码 0/1 |
...
| `bb_compare.py <a.fzz> <b.fzz>` | **面包板布局的度量尺**（在 `AuroraTessellation-NFC` 像素板项目里长出来，2026-09-27 收进库）：列**长度 / 段数 / 用孔数**，以及用户判据里的**「交集」= 交叉(X) + 搭线(T 形) + 穿元件** —— 三类各算 **1 次**（只数 X 会低估：同一张图实测 9 → 3）；另供 `pair_kind` / `obscures` / `hole_xy` 给其它工具**共用同一份几何**（防"两套实现各算各的、数字对不上"）|
| `part_box.py` | **元件几何**（同上收进库）：`canvas_mm`（画布尺寸→mm；★ `px` = **1/72 英寸**，不是 1/96 —— 按 96 算本体小 25%，实测"被 1010 板遮住的孔"因此漏判）、`body_box`（本体包围盒；内容跑到画布外时照实返回，不再罢工）、`place`（按实例矩阵搬到 sketch 绝对坐标）、`resolve_svg`（按 fzp 的 `breadboardView/layers@image` 找真 svg）、`pin_points`（引脚参考点，走完整祖先 `transform` 链：焊盘组的 `translate` 差 **200** 与组内 `x` 差 **−186** 会**互抵**，真脚距只有 **14.4**；有 `legId` 时取**腿尖**，core 件的连接点在腿上、不在 pad 标记上）|
| `diag_pair.py <fzz> <孔A> <孔B> [--drop Wire…] [--top N]` | **"这对孔到底能不能画成折线"的独立量尺**（2026-09-27 用户追问"你的问题在只会画斜线吧？"时长出来；**只读** ✓）：枚举 **直 / L / Z（偏移加密到 4.5 单位）/ 绕框 U 形**全部候选，逐条报【真实长 / 形状加权长 / 交集 / 成本】或**被哪条规则拒**（①自交 ②碰本体 ③重叠 ④盖住接线孔）。用途：把"生成器为何没选那条更好的走法"变成可读的数字 —— 实测 v62 的 5V `pin32C→pin47Y`：折线 68.6mm/交集 0 ⇒ 75.4 vs 斜线 73.2 ⇒ **只差 2.2** ⇒ 结论是"有折线、只是排序键让斜线赢了" ✓（不是几何上没路 ✗）。★ 判据与 `bb_compare` 同源 ✓（`pair_kind` / `seg_hits_box` / `obscures` / `hole_xy` ✓）、本体框与生成器同源 ✓（`part_box.body_box` + `place` ✓）；★ `seg_hits_box` **擦到框边也算碰** ✓（贴着模块边缘走会被拒 ✗ —— 手算采样验过：严格在框内 **0 点** ✓，是'物理上贴着外壳'的口径 ✓）；★ 唯一**抄**来的是 `shape_factor`（斜线罚 1.5 ✓，抄在脚本里并标了出处 ✗，只为对照 ✓）|
| `audit_green_board.py [仓根]` | **绿转接板相位审计**（2026-09-27 用户立规则时长出来 ✓）：新规矩（AGENTS §3b）—— **转接板不许影响「板外」孔的插拔** ✓ ⇒ 板边**正好压在孔行/孔列线上** ✗ 就会把那一排孔压掉一半（插不进线 ✗）。工具取该元件 `connector*pin`/`*leg` 的**中心**当基准线 ✓，量四条板边到最近基准线的距离 ⇒ 合格值 = **半格（离孔心 1.27mm）** ✓。★ 判据**必须锚在针脚上** ✗ —— 第一版写成"板的宽高是不是 2.54mm 的整数倍" ✗，那是错的 ✓（针脚本就不在板的整数格上）。现状：**36 个绿板件里 35 个违规** ✗（历史习惯把板做成整数格 ✗）|
| `bb_phase_raw.py <breadboard.svg>` | **看"未折叠"的板边相位** ✓（2026-09-27 ✓）：`audit_green_board.py` 报的是**折叠后**的相位 ✗（`min(d, 9−d)`）⇒ 看不出 0.28 到底是"差 0.28"还是"差 8.72" ✗，而补半格**要按方向挪** ✓。这个探针把四条边与**最近针脚线**的有符号偏移（草图单位 ✓）打出来，并给出"挪到半格 ±4.5 取小者"的建议 ✓。★ 它用 `viewBox` 原点做换算（原点必须减掉 ✓）；`FPC05-2H10PX` 就是靠它暴露"画布被整体上移过、绿板比画布还大" ✗ 这个反常情形的 ✓ |
| `check_inside_board.py <breadboard.svg> ...` | **改板边后必查** ✓（2026-09-27 ✓）：绿板（`fill=#00aa44` 的矩形）算板 ✓、其余图元的合并包围盒算本体 ✓（含 transform ✓）⇒ 报“本体是否还在板框内 ✓ + 四边余量（内部单位 ✓）”。用途：挪板边最容易出的错就是**把本体切到板外** ✗（板变小了、本体还在原处 ✗），这个探针一眼看出来 ✓ |
| `bb_inset.py <部件目录> [--dry]` | **给绿板元件加“板边让半格”的受限 codemod** ✓（2026-09-27 ✓，配合 AGENTS §3b 新规矩）：只认**精确写法**的板框 rect ✓，把它的几何换成四周各让 50（半格）✓ 并在 **模块 docstring 之后**插入 `INSET = 50` ✓；认不出就**报出来、一个字不动** ✗。★ 两个踩过的坑 ✓：① 插在“第一个空行”会插进 docstring 里 ✗ ⇒ 运行到板框直接 NameError ✗ ⇒ 改用 `ast` 找 docstring 结束行 ✓；② 曾支持 `%` 版写法 ✗ ⇒ LD1117 上生成器当场 `TypeError` ✗（几何改了、参数表没跟上 ✗）⇒ 该变体**已删除** ✓，这类件人工改 ✓。★ 改完必跑 `audit_green_board.py` + `check_inside_board.py` ✓ |
| `bb_legend3.py <sketch> <out> <x> <y> <行距> <首行偏移> <文字偏移> <模板.fzz>` | 往 sketch 的面包板视图里写**图例**（标题 + 每网一行：色条 + 网名；颜色取导线自己的 `wireExtras/@color`）；先清掉旧图例（文字件 + 色条）⇒ 可以反复重跑 |
| `file_sanity.py <sketch.fzz>` | sketch **文件级自检**：① 导线颜色必须来自 Fritzing 官方色表（表外 hex **画得对、但指示栏认不出、回退成"蓝"**，实测过）② 零长线 ③ 非法属性 ⇒ 有问题退出码 1 |
| `fix_connects.py <in.fzz> <out.fzz>` | 清**悬空 `<connect>` 记录**：删了导线/元件之后，孔上"接在已不存在的实例上"的记录还在 ⇒ Fritzing 里那些孔悬停时一片亮（实测我生成的版本各 12 处、用户手画版 0 处）⇒ 交付前清掉 |
...
### `.fzp` 里的 `<buses>`：文件里长什么样（2026-09-26 用户提示后补记 ✓）

**元件内部**「哪些脚本来就互通」就写在 `.fzp` 的 `<buses>` 里 ✓ —— 写元件时我们天天用它
（`AGENTS §3`：GND/3V3 等互通、`AGENTS §5`：同一元件多焊盘各自独立 connector + `<buses>` 互联 ✓），
但**它的文本形态**一直没记下来 ✗ ⇒ 读面包板零件时靠猜字段名 ✗，连错两轮 ✓。

| 事实 | 说明 |
|---|---|
| 元素名 | **`<buses>`**（里面每条 `<bus>` ✓）。★ 成员写法**不一定是** `<member connectorId=…>` ✗ |
| **稳健读法** ✓ | 遍历每个 `<bus>`，把**它里面所有带 `id` / `connectorId` 的子孙**都收进来 ✓（`member` / `connector` / 别的写法都吃 ✓）⇒ 不用赌字段名 ✓ |
| 面包板 **bus 数字**（可当自检 ✓）| `breadboard2.fzp`（830 孔全尺寸 ✓）：**130 条** = **126 条 × 5 孔**（每列 A–E 一条、F–J 一条 ✓）+ **4 条 × 50 孔**（最上一行 / 第二行 / 最下一行 / 倒数第二行，**各一整条** ✓）|
| 画面包板导线时 | 同一条 bus 内的孔**已经通** ✓ ⇒ **不必画跳线** ✗；跨 bus 才要跳线 ✓ |

★ **教训（用户 2026-09-26 指出）**：知识其实**已经在仓里**（`AGENTS` + 这个 README 都写过 `<buses>` ✓），
我却**没先去查** ✗，而是"猜一个字段名 → 失败 → 再猜一个" ✗ —— 两轮都花在这上面 ✓。
⇒ 规矩：**动手读/写某个 XML 结构前，先在仓里搜一遍**（`grep -r "buses" .` 一行的事 ✓）；
并且写规则时**不能只写"要做什么"**（"用 `<buses>` 声明互通" ✓），**必须带上"文件里长什么样"** ✓ ——
否则规则在要用的时候等于不存在 ✗。
